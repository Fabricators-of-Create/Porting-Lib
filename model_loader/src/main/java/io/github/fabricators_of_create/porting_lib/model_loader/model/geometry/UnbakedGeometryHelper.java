...
public class UnbakedGeometryHelper {
	...
	public static BakedModel bake(BlockModel blockModel, ModelBaker modelBaker, BlockModel owner, Function<Material, TextureAtlasSprite> spriteGetter, ModelState modelState, ResourceLocation modelLocation, boolean guiLight3d) {
		IUnbakedGeometry<?> customModel = blockModel.getGeometry().getCustomGeometry();
		if (customModel != null)
			return customModel.bake(blockModel.getGeometry(), modelBaker, spriteGetter, modelState, blockModel.getOverrides(modelBaker, owner, spriteGetter), modelLocation);

		Transformation rootTransform = blockModel.getGeometry().getRootTransform();
		...
		return blockModel.bake(modelBaker, owner, spriteGetter, modelState, modelLocation, guiLight3d);
	}
