...
public class TransferUtil {
	 ...
	 * @see TransferUtil#getItemStorage(World, BlockPos, BlockEntity, Direction)
	 */
	@Nullable
	public static Storage<ItemVariant> getItemStorage(World level, BlockPos pos, @Nullable Direction side) {
		return getItemStorage(level, pos, null, side);
	}

	/**
	 * Prefer {@link TransferUtil#getItemStorage(World, BlockPos, Direction)} variant when possible.
	 * @see TransferUtil#getItemStorage(World, BlockPos, BlockEntity, Direction)
	 */
	@Nullable
	public static Storage<ItemVariant> getItemStorage(World level, BlockPos pos) {
	 ...
	 * Prefer {@link TransferUtil#getItemStorage(World, BlockPos, Direction)} variant generally.
	 ...
	 * @see TransferUtil#getItemStorage(World, BlockPos, Direction)
	 * @return a Storage of ItemVariants, or null if none found.
	 */
	@Nullable
	public static Storage<ItemVariant> getItemStorage(World level, BlockPos pos, BlockEntity be, @Nullable Direction side) {
		if (be == null) {
		...
		boolean libOnly = level == null || level.isClient();
		if (libOnly) {
			// on the client we only allow lib handling.
			if (be instanceof ItemTransferable t) {
				boolean client = level != null && level.isClient();
				if (client) {
		...
		BlockState state = be == null ? level.getBlockState(pos) : be.getCachedState();
	 ...
	 * @see TransferUtil#getFluidStorage(World, BlockPos, BlockEntity, Direction)
	 */
	@Nullable
	public static Storage<FluidVariant> getFluidStorage(World level, BlockPos pos, @Nullable Direction side) {
		return getFluidStorage(level, pos, null, side);
	}

	/**
	 * Prefer {@link TransferUtil#getFluidStorage(World, BlockPos, Direction)} variant when possible.
	 * @see TransferUtil#getFluidStorage(World, BlockPos, BlockEntity, Direction)
	 */
	@Nullable
	public static Storage<FluidVariant> getFluidStorage(World level, BlockPos pos) {
	 ...
	 * Prefer {@link TransferUtil#getFluidStorage(World, BlockPos, Direction)} variant generally.
	 * @param level the Level to check in - may be client only, despite what regular FAPI allows. Null is allowed as long as 'be' is NOT null.
	 * @param pos the position to check. Null is allowed as long as 'be' is NOT null.
	 * @param be the Block Entity to check. Null is allowed as long as 'level' and 'pos' are NOT null.
	 * @param side the Direction to check in - null is considered all sides and will return a wrapper around all found.
	 * @see TransferUtil#getFluidStorage(World, BlockPos, Direction)
	 * @return a Storage of FluidVariants, or null if none found.
	 */
	@Nullable
	public static Storage<FluidVariant> getFluidStorage(World level, BlockPos pos, BlockEntity be, @Nullable Direction side) {
		if (be == null) {
			Objects.requireNonNull(level, "If a null Block Entity is provided, the Level may NOT be null!");
			Objects.requireNonNull(pos, "If a null Block Entity is provided, the pos may NOT be null!");
			be = level.getBlockEntity(pos);
	...
	public static List<FluidStack> getFluids(Storage<FluidVariant> storage, int cutoff) {
		...
		try (Transaction t = getTransaction()) {
			for (StorageView<FluidVariant> view : getNonEmpty(storage, t)) {
	...
	public static List<ItemStack> getItems(Storage<ItemVariant> storage, int cutoff) {
		...
		try (Transaction t = getTransaction()) {
			for (StorageView<ItemVariant> view : getNonEmpty(storage, t)) {
				long contained = view.getAmount();
				ItemVariant item = view.getResource();
				int maxSize = item.getItem().getMaxCount();
	...
	public static <T> boolean clearStorage(Storage<T> storage) {
		...
		try (Transaction t = getTransaction()) {
			Iterator<? extends StorageView<T>> itr = getNonEmpty(storage, t).iterator();
	...
	public static FluidStack extractAnyFluid(Storage<FluidVariant> storage, long maxAmount, Transaction tx) {
		...
		for (StorageView<FluidVariant> view : getNonEmpty(storage, tx)) {
	...
	public static long insertToMainInv(PlayerEntity player, ItemVariant variant, long amount) {
		long inserted = 0;
		try (Transaction t = getTransaction()) {
			...
			for (int i = 9; i < PlayerInventory.MAIN_SIZE; i++) { // start at 9 and skip hotbar, end before armor and offhand
	...
	public static List<ItemStack> extractAllAsStacks(Storage<ItemVariant> storage) {
		...
		try (Transaction t = getTransaction()) {
			Iterator<? extends StorageView<ItemVariant>> itr = getNonEmpty(storage, t).iterator();
			...
			while (currentView != null) {
				...
				int max = (int) Math.min(contained, variant.getItem().getMaxCount());
	...
	public static BlockApiCache<Storage<ItemVariant>, Direction> getItemCache(World level, BlockPos pos) {
	 ...
	 */
	@Deprecated(forRemoval = true)
	public static BlockApiCache<Storage<FluidVariant>, Direction> getFluidCache(World level, BlockPos pos) {
		if (level instanceof ServerWorld server) {
