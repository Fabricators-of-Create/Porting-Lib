...
public class VertexUtils {
	...
	public static void putBulkData(VertexConsumer builder, MatrixStack.Entry poseStack, BakedQuad bakedQuad, float red, float green, float blue, int packedLight, int packedOverlay, boolean readExistingColor) {
	...
	public static void putBulkData(VertexConsumer builder, MatrixStack.Entry pose, BakedQuad bakedQuad, float red, float green, float blue, float alpha, int packedLight, int packedOverlay) {
	...
	// Copy of putBulkData with alpha support
	public static void putBulkData(VertexConsumer builder, MatrixStack.Entry pose, BakedQuad bakedQuad, float red, float green, float blue, float alpha, int packedLight, int packedOverlay, boolean readExistingColor) {
	...
	// Copy of putBulkData with alpha support
	public static void putBulkData(VertexConsumer builder, MatrixStack.Entry pose, BakedQuad bakedQuad, float[] baseBrightness, float red, float green, float blue, float alpha, int[] lightmap, int packedOverlay, boolean readExistingColor) {
		int[] aint = bakedQuad.getVertexData();
		Vec3i faceNormal = bakedQuad.getFace().getVector();
		Vec3f normal = new Vec3f((float)faceNormal.getX(), (float)faceNormal.getY(), (float)faceNormal.getZ());
		Matrix4f matrix4f = pose.getModel();
		normal.transform(pose.getNormal());
		...
		try (MemoryStack memorystack = MemoryStack.stackPush()) {
			...
			for(int v = 0; v < vertexCount; ++v) {
				...
				applyBakedNormals(normal, bytebuffer, pose.getNormal());
				builder.vertex(pos.getX(), pos.getY(), pos.getZ(), cr, cg, cb, ca, f9, f10, packedOverlay, lightmapCoord, normal.getX(), normal.getY(), normal.getZ());
			}
		}
	}
	...
	public static void applyBakedNormals(Vec3f generated, ByteBuffer data, Matrix3f normalTransform) {
		byte nx = data.get(28);
		byte ny = data.get(29);
		byte nz = data.get(30);
		if (nx != 0 || ny != 0 || nz != 0) {
