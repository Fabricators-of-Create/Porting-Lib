...
public class GuiUtils { // name is this to maintain max compat with upstream
	...
	public static void drawGradientRect(Matrix4f matrix, int z, int left, int top, int right, int bottom, int startColor, int endColor) {
		...
		buffer.vertex(matrix, right,    top, z).color(startRed, startGreen, startBlue, startAlpha).next();
		buffer.vertex(matrix,  left,    top, z).color(startRed, startGreen, startBlue, startAlpha).next();
		buffer.vertex(matrix,  left, bottom, z).color(  endRed,   endGreen,   endBlue,   endAlpha).next();
		buffer.vertex(matrix, right, bottom, z).color(  endRed,   endGreen,   endBlue,   endAlpha).next();
		tessellator.draw();
	...
	public static void drawHoveringText(@NotNull final ItemStack stack, MatrixStack mStack, List<? extends StringVisitable> textLines, int mouseX, int mouseY, int screenWidth, int screenHeight, int maxTextWidth, int backgroundColor, int borderColorStart, int borderColorEnd, TextRenderer font) {
	 ...
	 */
	public static void drawContinuousTexturedBox(MatrixStack poseStack, Identifier res, int x, int y, int u, int v, int width, int height, int textureWidth, int textureHeight,
	 ...
	 * @param zLevel the zLevel to draw at
	 */
	public static void drawContinuousTexturedBox(MatrixStack poseStack, int x, int y, int u, int v, int width, int height, int textureWidth, int textureHeight,
												 int topBorder, int bottomBorder, int leftBorder, int rightBorder, float zLevel) {
	...
	public static void drawTexturedModalRect(MatrixStack poseStack, int x, int y, int u, int v, int width, int height, float zLevel) {
		...
		Matrix4f matrix = poseStack.peek().getModel();
