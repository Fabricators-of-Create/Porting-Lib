...
public class LightUtil {
	...
	public static void pack(float[] from, int[] to, VertexFormat formatTo, int v, int e) {
		...
		VertexFormatElement.DataType type = element.getDataType();
		int size = type.getByteLength();
		...
		for (int i = 0; i < 4; i++) {
			if (i < count) {
				...
				if (type == VertexFormatElement.DataType.FLOAT) {
					bits = Float.floatToRawIntBits(f);
				} else if (
						type == VertexFormatElement.DataType.UBYTE ||
								type == VertexFormatElement.DataType.USHORT ||
								type == VertexFormatElement.DataType.UINT
				) {
