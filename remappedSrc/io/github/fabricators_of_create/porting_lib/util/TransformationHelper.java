...
public final class TransformationHelper
{
	...
	public static Quaternion quatFromXYZ(Vec3f xyz, boolean degrees) {
	...
	public static Vec3f lerp(Vec3f from, Vec3f to, float progress) {
		Vec3f res = from.copy();
	...
	public static AffineTransformation slerp(AffineTransformation one, AffineTransformation that, float progress) {
		return new AffineTransformation(
				...
				slerp(one.getRotation1(), that.getRotation1(), progress)
		);
	}
	...
	public static class Deserializer implements JsonDeserializer<AffineTransformation> {
		private static final Vec3f ORIGIN_CORNER = new Vec3f();
		private static final Vec3f ORIGIN_OPPOSING_CORNER = new Vec3f(1f, 1f, 1f);
		private static final Vec3f ORIGIN_CENTER = new Vec3f(.5f, .5f, .5f);
		...
		public AffineTransformation deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
			...
			Vec3f origin = ORIGIN_OPPOSING_CORNER;
			if (obj.has("translation")) {
				float[] vec = parseFloatArray(obj.get("translation"), 3, "Translation");
				translation = new Vec3f(vec[0], vec[1], vec[2]);
			...
			if (obj.has("scale")) {
				if(!obj.get("scale").isJsonArray()) {
					...
					scale = new Vec3f(vec[0], vec[1], vec[2]);
				}
				obj.remove("scale");
			}
			...
			if (!ORIGIN_CENTER.equals(origin)) {
				Vec3f originFromCenter = origin.copy();
		...
		private static Vec3f parseOrigin(JsonObject obj) {
			Vec3f origin = null;
			...
			if (originElement.isJsonArray()) {
				...
				origin = new Vec3f(vec[0], vec[1], vec[2]);
			}
		...
		public static Quaternion parseAxisRotation(JsonElement e)
		{
			...
			try
			{
				...
				{
					ret = Vec3f.POSITIVE_X.getDegreesQuaternion(entry.getValue().getAsNumber().floatValue());
				}
				else if (entry.getKey().equals("y"))
				{
					ret = Vec3f.POSITIVE_Y.getDegreesQuaternion(entry.getValue().getAsNumber().floatValue());
				}
				else if (entry.getKey().equals("z"))
				{
					ret = Vec3f.POSITIVE_Z.getDegreesQuaternion(entry.getValue().getAsNumber().floatValue());
				}
