...
public final class DynamicBucketModel implements IModelGeometry<DynamicBucketModel> {
	...
	public BakedModel bake(IModelConfiguration owner, ModelLoader bakery, Function<SpriteIdentifier, Sprite> spriteGetter, ModelBakeSettings modelTransform, ModelOverrideList overrides, Identifier modelLocation) {
		...
		Sprite fluidSprite = fluid != Fluids.EMPTY ? spriteGetter.apply(new SpriteIdentifier(SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE, fluid.getAttributes().getStillTexture())) : null;
			...
			return TransferUtil.getFluidContained(stack)
					.map(fluidStack -> {
						...
						if (!cache.containsKey(name)) {
							...
							BakedModel bakedModel = unbaked.bake(owner, bakery, SpriteIdentifier::getSprite, ModelRotation.X0_Y0, this, new Identifier("forge:bucket_override"));
