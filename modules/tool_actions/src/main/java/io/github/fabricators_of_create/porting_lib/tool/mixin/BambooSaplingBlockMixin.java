...
public class BambooSaplingBlockMixin {
	...
	private ItemStack toolActionIsSword(ItemStack original) {
		if (original.getItem() instanceof ToolActionItem)
			return original.canPerformAction(ToolActions.SWORD_DIG) ? original.getItem() instanceof SwordItem ? original : Items.IRON_SWORD.getDefaultInstance() : Items.AIR.getDefaultInstance();
		return original;
	}
}
