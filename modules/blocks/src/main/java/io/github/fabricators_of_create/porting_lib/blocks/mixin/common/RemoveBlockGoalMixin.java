...
public class RemoveBlockGoalMixin {
	...
	private boolean checkEntityCanRemoveBlock(boolean original, LevelReader level, BlockPos pos, @Local ChunkAccess chunk) {
		BlockState state = chunk.getBlockState(pos);
		if (state.getBlock() instanceof EntityDestroyBlock block) {
			return original && block.canEntityDestroy(state, level, pos, this.removerMob);
		}
