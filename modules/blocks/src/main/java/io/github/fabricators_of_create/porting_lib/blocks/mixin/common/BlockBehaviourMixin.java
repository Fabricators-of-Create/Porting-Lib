...
public class BlockBehaviourMixin {
	...
	private void onBlockExploded(Block instance, ServerLevel level, BlockPos pos, Explosion explosion, Operation<Void> original, @Local(argsOnly = true) BlockState state) {
		if (state.getBlock() instanceof OnExplodedBlock onExplodedBlock) {
			onExplodedBlock.onBlockExploded(state, level, pos, explosion);
		} else {
	...
	private boolean dontSetBlockOnCustom(ServerLevel instance, BlockPos pos, BlockState blockState, int i, @Local(argsOnly = true) BlockState state) {
	...
	private boolean doHarvestCheck(Player instance, BlockState state, Operation<Boolean> original, @Local(argsOnly = true) BlockGetter level, @Local(argsOnly = true) BlockPos pos) {
