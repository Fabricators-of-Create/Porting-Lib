...
public class DeferredRegister<T> {
	...
	/**
	 * DeferredRegister factory for custom registries or {@link BuiltInRegistries vanilla registries} to lookup based on the provided registry name. Supports both registries that already exist or do not exist yet.
	 * <p>
	 * If the registry is never created, any {@link DeferredHolder}s made from this DeferredRegister will throw an exception.
	 *
	...
	/**
	 * Creates a tag key based on the provided identifier and the registry name linked to this DeferredRegister. To use the {@linkplain #getNamespace() current namespace} as the tag key namespace automatically, use {@link #createTagKey(String)}.
	 *
...
//		this.aliases.forEach(registry::addAlias);
	...
	private void addRegistry() {
		...
		if (this.customRegistry != null) {
			RegistryAttributeHolder.get(this.customRegistry.key()).addAttribute(RegistryAttribute.MODDED);
			((WritableRegistry) BuiltInRegistries.REGISTRY).register(this.customRegistry.key(), this.customRegistry, RegistrationInfo.BUILT_IN);
		}
	}
	...
	private static class RegistryHolder<V> implements Supplier<Registry<V>> {
		...
		public @Nullable Registry<V> get() {
			...
			if (this.registry == null)
				this.registry = (Registry<V>) BuiltInRegistries.REGISTRY.getValue(this.registryKey.identifier());
