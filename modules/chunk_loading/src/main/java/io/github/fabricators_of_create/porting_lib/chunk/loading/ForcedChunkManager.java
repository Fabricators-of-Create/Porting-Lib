...
public class ForcedChunkManager {
	...
	private static Map<ResourceLocation, TicketController> controllers = new HashMap<>();
	...
	public static boolean hasForcedChunks(ServerLevel level) {
		...
		return !data.getForceLoadedChunks().isEmpty() || !data.port_lib$getBlockForcedChunks().isEmpty() || !data.port_lib$getEntityForcedChunks().isEmpty();
	}

	// Method for vanilla
	public static boolean hasForcedChunks(ServerLevel level, LongSet forcedChunks, Operation<Boolean> original) {
		TicketStorage data = level.getDataStorage().get(TicketStorage.TYPE);
		if (data == null) return false;
		return !original.call(forcedChunks) || !data.port_lib$getBlockForcedChunks().isEmpty() || !data.port_lib$getEntityForcedChunks().isEmpty();
	}
	...
	public static void activateAllDeactivatedTickets(ServerLevel level, TicketStorage saveData) {
		TicketTracker<BlockPos> blockForcedChunks = saveData.port_lib$getBlockForcedChunks();
		TicketTracker<UUID> entityForcedChunks = saveData.port_lib$getEntityForcedChunks();
	...
	public record OwnedChunks(ResourceLocation controller, Map<BlockPos, TicketSet> blockChunks, Map<UUID, TicketSet> entityChunks) {
		private static final Codec<Map<BlockPos, TicketSet>> BLOCK_CHUNK_CODEC = PortingLibExtraCodecs.unboundedMapAsList("position", BlockPos.CODEC, "tickets", TicketSet.CODEC);
		private static final Codec<Map<UUID, TicketSet>> ENTITY_CHUNK_CODEC = PortingLibExtraCodecs.unboundedMapAsList("uuid", UUIDUtil.CODEC, "tickets", TicketSet.CODEC);
	...
	public static App<Mu<TicketStorage>, List<OwnedChunks>> defineExtraStorageParams() {
		return OwnedChunks.CODEC.listOf().optionalFieldOf("neo_ticket_data", List.of()).forGetter(storage -> {
			...
			Map<ResourceLocation, Map<BlockPos, TicketSet>> blockTickets = gatherTicketsById(storage.port_lib$getBlockForcedChunks(), true, true);
			Map<ResourceLocation, Map<UUID, TicketSet>> entityTickets = gatherTicketsById(storage.port_lib$getEntityForcedChunks(), true, true);
	...
	public static TicketStorage readStoredTickets(Function<List<Pair<ChunkPos, Ticket>>, TicketStorage> vanillaInitializer, List<Pair<ChunkPos, Ticket>> tickets,
		...
		TicketTracker<BlockPos> blockForcedChunks = ticketStorage.port_lib$getBlockForcedChunks();
		TicketTracker<UUID> entityForcedChunks = ticketStorage.port_lib$getEntityForcedChunks();
