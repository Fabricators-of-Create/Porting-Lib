...
public abstract class MobSpawnEvents extends EntityEvent {
	 ...
	 * <p>
	 */
	public static Event<FinalizeSpawnCallback> FINALIZE_SPAWN = EventFactory.createArrayBacked(FinalizeSpawnCallback.class, callbacks -> event -> {
		for (FinalizeSpawnCallback e : callbacks)
			e.onFinalizeSpawn(event);
	});
	 ...
	 * @author cpw
	 */
	public static Event<AllowDespawnCallback> ALLOW_DESPAWN = EventFactory.createArrayBacked(AllowDespawnCallback.class, callbacks -> event -> {
		for (AllowDespawnCallback e : callbacks)
			e.onAllowDespawn(event);
	});
	...
	/**
	 * @apiNote Do not construct directly. Access via {@link PortingHooks#onFinalizeSpawn}.
	 */
	@ApiStatus.Internal
	public MobSpawnEvents(Mob mob, ServerLevelAccessor level, double x, double y, double z) {
		super(mob);
		this.level = level;
		this.x = x;
		this.y = y;
	...
	public static class FinalizeSpawn extends MobSpawnEvents {
		...
		public void sendEvent() {
			FINALIZE_SPAWN.invoker().onFinalizeSpawn(this);
		}
	}
	...
	public static class AllowDespawn extends MobSpawnEvents {
		...
		@Override
		public void sendEvent() {
			ALLOW_DESPAWN.invoker().onAllowDespawn(this);
		}
	}
	...
	public interface AllowDespawnCallback {
		void onAllowDespawn(AllowDespawn event);
	}
}
