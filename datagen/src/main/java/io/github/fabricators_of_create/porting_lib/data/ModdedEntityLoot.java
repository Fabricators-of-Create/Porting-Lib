...
public abstract class ModdedEntityLoot extends EntityLootSubProvider {

	protected ModdedEntityLoot(FeatureFlagSet featureFlagSet) {
		...
		for(EntityType<?> entityType : getKnownEntities()) {
			if (entityType.isEnabled(this.enabledFeatures)) {
				...
				if (isNonLiving(entityType)) {
					...
					if (map != null) {
						map.forEach((resourceLocationx, builder) -> {
							if (!set.add(resourceLocationx)) {
								throw new IllegalStateException(String.format(Locale.ROOT, "Duplicate loottable '%s' for '%s'", resourceLocationx, Registry.ENTITY_TYPE.getKey(entityType)));
							} else {
					...
					if (map != null) {
						throw new IllegalStateException(String.format(Locale.ROOT, "Weird loottables '%s' for '%s', not a LivingEntity so should not have loot", map.keySet().stream().map(ResourceLocation::toString).collect(Collectors.joining(",")), Registry.ENTITY_TYPE.getKey(entityType)));
					}
				}
