 ...
 */
public final class ToolActionsLootTableProvider extends ModdedLootTableProvider {
	...
	public List<SubProviderEntry> getTables() {
		...
		return super.getTables().stream().map(entry -> {
			...
			return new SubProviderEntry(() -> replaceAndFilterChangesOnly(entry.provider().get()), entry.paramSet());
	...
	private <T, C> T getPrivateValue(Class<C> clazz, C inst, String name) { // We are the only ones running this in dev, so we are just going to ignore mappings
		try {
			Field field = clazz.getDeclaredField(name);
			field.setAccessible(true);
			T value = (T) field.get(inst);
