...
public class PartialNBTIngredient extends AbstractIngredient {
	...
	public JsonElement toJson() {
		...
		if (items.size() == 1) {
			json.addProperty("item", BuiltInRegistries.ITEM.getKey(items.iterator().next()).toString());
		} else {
			JsonArray items = new JsonArray();
			// ensure the order of items in the set is deterministic when saved to JSON
			this.items.stream().map(BuiltInRegistries.ITEM::getKey).sorted().forEach(name -> items.add(name.toString()));
